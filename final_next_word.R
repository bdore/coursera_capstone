...
uniProb_fn <- function(x, bigram = NULL, notFoundResults = 5){
  ...
  if (identical(nxt_word_df$Freq, integer(0))){
    ...
    # if less than 5 unigrams are found, backoff one word in the sentence
    # print(x)
...
biProb_fn <- function(x){
  nxt_word_df <- ngramlist[[3]][grep(paste("^", x, " ", sep = ""), ngramlist[[3]]$ngram), c("ngram","Freq")]
  # if no trigrams are found, look for bigrams beggining with last word (backoff)
  if (identical(nxt_word_df$Freq, integer(0))){
    # print(x)
    x <- word(x, -1)
    # print(x)
    ...
    not_found <- sum(is.na(biProb$Freq))
    # if less than 5 bigrams are found, backoff
    if (not_found != 0){
      # print(x)
      not_found <- uniProb_fn(word(x, -1), x, not_found)
    }
    # print(x)
...
triProb_fn <- function(x){
  ...
  if (identical(nxt_word_df$Freq, integer(0))){
    ...
    not_found <- sum(is.na(triProb$Freq))
    # if less than 5 bigrams are found, backoff
    if (not_found != 0){
      # print(x)
      not_found <- biProb_fn(word(x, -1), x, not_found)
    }
    # print(x)
