...
lines_sampled <- numReadLines
save(lines_sampled, file="ngramlist_003_lines_sampled.RData")
save(ngramlist, file="ngram_data_raw.RData")
load("ngram_data_raw.RData")

View(arrange(ngramlist[[1]], desc(Freq)))
...
for(i in 1:length(ngramlist)){
  ...
  # keep n-grams with frequency > 1
  #ngramlist[[i]] <- ngramlist[[i]][ngramlist[[i]]$Freq != 1,]
  
}

save(ngramlist, file="ngram_data_freq1_keep.RData")
